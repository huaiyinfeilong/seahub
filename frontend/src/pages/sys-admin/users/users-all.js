...
class Content extends Component {
  ...
  render() {
    const { loading, errorMsg, items, pageInfo, availableRoles, isAllUsersSelected, curPerPage } = this.props;
    if (loading) {
      return <Loading />;
    } else if (errorMsg) {
...
class Item extends Component {
  ...
  updateRole = (role) => {
    seafileAPI.sysAdminUpdateUserInfo('role', role, this.props.item.email).then(res => {
      this.setState({
        role: role
      });
  ...
  render() {
    let { status, role, quota_total, isOpIconShown, isActivateWaitingDialogOpen, isSetQuotaDialogOpen,
      isDeleteUserDialogOpen, isResetUserPasswordDialogOpen, isResetUserPasswordWaitingDialogOpen } = this.state;
...
class UsersAll extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      isImportWaitingDialogOpen: false,
      isAddUserWaitingDialogOpen: false
    };
  }
  ...
  toggleImportWaitingDialog = () => {
    this.setState({isImportWaitingDialogOpen: !this.state.isImportWaitingDialogOpen});
  }

  closeImportWaitingDialog = () => {
    this.setState({isImportWaitingDialogOpen: false});
  }

  toggleAddUserWaitingDialog = () => {
    this.setState({isAddUserWaitingDialogOpen: !this.state.isAddUserWaitingDialogOpen});
  }

  closeAddUserWaitingDialog = () => {
    this.setState({isAddUserWaitingDialogOpen: false});
  }
  ...
  getUsersListByPage = (page) => {
    ...
    seafileAPI.sysAdminListAllUsers(page, perPage).then(res => {
  ...
  addUser = (newUserInfo) => {
    this.toggleAddUserWaitingDialog();
    let { email, name, role, password, isActive } = newUserInfo;
    seafileAPI.sysAdminAddNewUser(email, name, role, password, isActive).then((res) => {
      let oldUserList = this.state.userList;
      oldUserList.push(res.data);
      this.setState({
        userList: oldUserList
      });
      this.toggleAddUserDialog();
      this.closeAddUserWaitingDialog();
      toaster.success(gettext('success'));
    }).catch((error) => {
      let errMsg = Utils.getErrorMsg(error);
      toaster.danger(errMsg);
  ...
  render() {
    let { isImportWaitingDialogOpen, isAddUserWaitingDialogOpen, hasUserSelected, isImportUserDialogOpen,
      isAddUserDialogOpen, isBatchDeleteUserDialogOpen, isBatchSetQuotaDialogOpen } = this.state;
    return (
      <Fragment>
        <MainPanelTopbar>
          {hasUserSelected ?
            ...
            <Fragment>
              ...
              <Button className="btn btn-secondary operation-item" ><a href={siteRoot+'sys/useradmin/export-excel/'}>{gettext('Export Excel')}</a></Button>
            </Fragment>
          }
        </MainPanelTopbar>
        <div className="main-panel-center flex-row">
          <div className="cur-view-container">
            <UsersNav currentItem="database" />
        ...
        {isImportWaitingDialogOpen &&
        <CommonWaitingDialog
          toggle={this.toggleImportWaitingDialog}
          message={gettext('Importing..., please wait')}
        />
        }
        {isAddUserWaitingDialogOpen &&
        <CommonWaitingDialog
          toggle={this.toggleAddUserWaitingDialog}
          message={gettext('Adding..., please wait')}
        />
        }
