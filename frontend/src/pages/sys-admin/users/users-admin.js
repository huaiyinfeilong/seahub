...
class Content extends Component {
  ...
  render() {
    const { loading, errorMsg, items, availableAdminRoles, isAllUsersSelected } = this.props;
    if (loading) {
      return <Loading />;
    } else if (errorMsg) {
...
class Item extends Component {
  ...
  render() {
    ...
    let {item, availableAdminRoles } = this.props;
...
class UsersAdmin extends Component {
  ...
  getUsersList = () => {
    seafileAPI.sysAdminListAllAdminUsers().then(res => {
      let users = res.data.admin_users.map(user => {return new SysAdminUser(user);});
      this.setState({
        userList: users,
        availableAdminRoles: res.data.available_admin_roles,
        loading: false
      });
    }).catch((error) => {
      if (error.response) {
  ...
  addAdminInBatch = (emailString) => {
    seafileAPI.sysAdminAddAdminInBatch(emailString).then(res => {
      let users = res.data.new_admin_info.map(user => {return new SysAdminUser(user);});
      let oldUserList = this.state.userList;
          ...
          <div className="cur-view-container">
            ...
            <div className="cur-view-content">
              <Content
                ...
                availableAdminRoles={this.state.availableAdminRoles}
